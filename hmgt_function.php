...
add_action( 'wp_ajax_hmgt_change_profile_photo',  'hmgt_change_profile_photo');
add_action( 'wp_ajax_hmgt_get_appliyed_bad',  'hmgt_get_appliyed_bad');


function hmgt_change_profile_photo()
{
	?>
...
function hmgt_get_appliyed_bad(){
	...
	if(!empty($result['0']->discharge_time)){
		...
		<?php }else{ 
			$patientdetails = get_user_detail_byid($result[0]->patient_id);
		...
		<script>
			...
			//$('#discharge_time').val("");
			swal("","<?php print "This Bed Already Assigned To ".$name ?>.","error");
		</script>
		<?php }
	}else{?>
	<script>
	...
	if(!empty($result)){ ?>
		<script>
			$("#save_allow").attr('disabled',true);
			$('#discharge_time').val("");
			swal("Opps..","<?php print "This Bad Already Assign To ".$name ?>.","error");
		</script>
	<?php } else {?> 
...
add_action('init','hmgt_frontend_menu_list');
//--------End nurse notes-------------------------
...
function hmgt_get_medicine_take_timelabel($id)
{
	if($id=='after_meal')
                    ...
                    <div class="panel-body">
                        ...
                        if(!empty($all_medicine_list))
                        {
                        	...
                        	<tr>
											...
											<td><?php echo $retrieved_data->time; ?></td>
											<td><?php echo $retrieved_data->per_days; ?></td>
											<td><?php echo hmgt_get_medicine_take_timelabel($retrieved_data->takes_time); ?></td>
											</tr>
                        	<?php 	
                        	}
                        } ?>
...
function datatable_multi_language()
{
...
function hmgt_frontend_menu_list()
{
