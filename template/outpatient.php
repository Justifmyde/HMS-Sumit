		...
		$patientdata=get_users($get_patient);
		 ...
		 if(!empty($patientdata))
		 {
		 	foreach ($patientdata as $retrieved_data){
				if($obj_hospital->role == 'patient' && get_option('hmgt_enable_outpatientlist_for_outpatient')=='no')
				{
					$patient_type=get_user_meta($retrieved_data->ID,'patient_type',true);
					if($patient_type=='outpatient' && $current_user_id==$retrieved_data->ID)
					{ ?>
						<tr>
