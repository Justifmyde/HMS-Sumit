 	...
 	echo $obj_message->hmgt_pagination($totlal_message,$p,$prev,$next,'dashboard=user&page=message&tab=inbox');?>
 		...
 		foreach($message as $msg)
 		{
            ...
            <td><?php echo hmgt_get_display_name($msg->sender);?></td>
             <td>
                 <a href="?dashboard=user&page=message&tab=inbox&tab=view_message&from=inbox&id=<?php echo $msg->message_id;?>"> <?php echo $msg->msg_subject;if($obj_message->hmgt_count_reply_item($msg->post_id)>=1){?><span class="badge badge-success pull-right"><?php echo $obj_message->hmgt_count_reply_item($msg->post_id);?></span><?php } ?></a>
            </td>
